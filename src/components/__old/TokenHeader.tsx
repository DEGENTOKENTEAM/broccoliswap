...
const CopyableContractAddress = (props: {
    address: string;
    network: string;
}) => {
    ...
    return (
        <>
            <span>
                Contract address: {props.address.substring(0, 5)}...
                {props.address.substring(props.address.length - 3)}
            </span>
            <RiFileCopyFill
                className="inline ml-1 cursor-pointer -mt-1"
                onClick={copyAddress}
            />
            <a
            ...
            <span
                className={classNames(
                    "ml-1",
                    spanCopiedVisible ? "opacity-1" : "opacity-0"
                )}
            >
                Copied!
            </span>
        ...
        <div className="flex flex-col items-start h-full py-1 pl-3 text-slate-200">
            <div className="flex w-full items-start">
                        ...
                        <span className="text-xs">
                            <MarketcapDisplay
                                marketCap={
                                    ...
                                    info.market_data?.fully_diluted_valuation
                                        .usd
                                }
                            />
                            <Volume24H
                ...
                <div className="flex mt-1 text-xl">
                    {info.links?.homepage.find(page => !!page) && (
                        <a
                            className="px-1.5"
                            href={`${info.links?.homepage.find(
                                page => !!page
                            )}`}
                            target="_blank"
                            rel="noreferrer"
                    ...
                    {info.links?.telegram_channel_identifier && (
                        <a
                            className="px-1.5"
                            href={`https://t.me/${info.links?.telegram_channel_identifier}`}
                            target="_blank"
                            rel="noreferrer"
                        >
                    ...
                    {info.links?.chat_url.find(url =>
                        url.includes("//discord.com")
                    ) && (
                        <a
                            className="px-1.5"
                            href={info.links?.chat_url.find(url =>
                                url.includes("//discord.com")
                            )}
                            target="_blank"
                            rel="noreferrer"
