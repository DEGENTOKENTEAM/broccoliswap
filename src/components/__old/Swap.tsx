...
import {
    ...
    rubicRPCEndpoints,
    rubicTokenNetworkToBlockchain,
    rubicTokenNetworkToChainId,
    SearchResult,
    SwapSide,
    Token,
    wagmiNetworkIdToRubicNetwork
} from "@/__old__types";
...
const ActiveToken = (props: {
    ...
    const { isLoading: balanceIsLoading, data: balanceData } = useBalance({
        ...
        chainId:
            rubicTokenNetworkToChainId[
                props.token.network as keyof typeof rubicTokenNetworkToChainId
            ]
    });
            ...
            <div className="flex-grow">
                <div className="flex flex-col">
                    <div className="text-xs -mb-1.5">
                        {
                            (rubicNetworkToBitqueryNetwork as any)[
                                props.token.network
                            ]
                        }
                    </div>
                                        ...
                                        <span>
                                            ...
                                            <span
                                                onClick={() => {
                                                    ...
                                                    setAmountInputRef.current!.value =
                                                        balanceData.formatted;
                                                }}
                ...
                {props.setAmount && (
                    <div>
                        <input
                            ...
                            defaultValue={
                                props.amount ? props.amount : undefined
                            }
...
const OtherToken = (props: {
    ...
    const { isLoading: balanceIsLoading, data: balanceData } = useBalance({
        address,
        token:
            props.token.address !== "0x0000000000000000000000000000000000000000"
                ? props.token.address
                : undefined,
        chainId:
            rubicTokenNetworkToChainId[
    ...
    return (
        <div className="relative bg-slate-900 border border-orange-900 p-3 my-2 flex items-center gap-2 text-sm group">
            ...
            <div
                className="absolute top-0 right-0 group-hover:opacity-100 opacity-0 cursor-pointer text-white bg-orange-600 w-5 h-5 flex items-center justify-center p-0"
...
const OtherTokenSelector = (props: {
    otherToken?: Token | null;
    setOtherToken: (token: Token | null) => void;
    amount?: number;
    setAmount?: (amount: number) => void;
}) => {
    return (
        <>
            {props.otherToken ? (
                <OtherToken
                    ...
                    token={props.otherToken}
                    removeToken={() => props.setOtherToken(null)}
                />
            ) : (
                <SearchToken
                    setActiveToken={props.setOtherToken}
                    inputClassName="py-2 my-2"
                    includeNative={true}
                />
            )}
        </>
...
const getTrades = async (
    ...
    const fromToken =
        swapSide === SwapSide.LEFT
            ? {
                  blockchain: (rubicTokenNetworkToBlockchain as any)[
                      activeToken.network
                  ],
    ...
    try {
        const wrappedTrade = await calculateBestTrade(
            sdk,
            slippage,
            fromToken,
            amount,
...
export const Swap = (props: { activeToken: Token; className?: string }) => {
    ...
    const [trade, setTrade] = useState<
        OnChainTrade | CrossChainTrade | string
    >();
    ...
    return (
        <div
            className={classNames(
                "text-slate-200 border-l border-zinc-800 h-full px-3",
                props.className
            )}
        >
            ...
            {trade && typeof trade === "object" && (
                <SwapDetails
                    inputAmount={toPrecision(
                        trade.from.tokenAmount.toNumber(),
                        4
                    )}
                    outputAmount={toPrecision(
                        trade.to.tokenAmount.toNumber(),
                        4
                    )}
                    inputName={trade.from.symbol}
                    outputName={trade.to.symbol}
                    slippage={slippage}
                    minimumOutputAmount={toPrecision(
                        getMinimumOutputAmount(trade).toNumber(),
                        4
                    )}
