...
const fetchTokens = async (
    ...
    const result = await fetch(
        `https://tokens.rubic.exchange/api/v1/tokens?symbol=${filter}&networks=${networks}&pageSize=10`
    );
...
const SearchResultItem = (props: {
    ...
    const { isLoading: balanceIsLoading, data: balanceData } = useBalance({
        ...
        chainId:
            rubicTokenNetworkToChainId[
                props.searchResult
                    .blockchainNetwork as keyof typeof rubicTokenNetworkToChainId
            ]
    });
            ...
            <div className="flex items-center">
                        ...
                        <div className="font-thin">
                            {
                                (rubicNetworkToBitqueryNetwork as any)[
                                    props.searchResult.blockchainNetwork
                                ]
                            }
                        </div>
                ...
                <div className="pr-3 font-bold">
                    <div className="flex flex-col">
                        ...
                        <div className="font-thin">
                            {props.searchResult.usdPrice &&
                                `${formatUsd.format(
                                    parseFloat(props.searchResult.usdPrice)
                                )}`}
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col">
                        {balanceIsLoading && (
                            <div className="flex items-center gap-1">
                                <div>Balance:</div>
                                <div>
                                    <Spinner className="w-3 h-3 m-0 p-0 " />
                                </div>
                            </div>
                        )}
                        {!balanceIsLoading &&
                            balanceData &&
                            balanceData.value.gt(0) &&
                            `Balance: ${toPrecision(
...
export const SearchToken = (props: {
    includeNative: boolean;
        ...
        >
            <input
                ...
                className={classNames(
                    "w-full border border-gray-800 focus:border-orange-900 p-1 text-xs bg-[#181818] text-gray-400 focus:outline-none",
